...
held, `access.villager.TraderFreeze` predicts the resulting name from the item and locks the mob
(no-AI / invulnerable, marked with the `headvault.frozen` tag) if it becomes a trader, reverting it
on a rename that no longer matches — while vanilla still performs the actual rename.
